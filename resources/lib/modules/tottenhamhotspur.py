...
def TrendingGrid(fromPage,toPage,tagIds,itemsPerGrid,excludeArticleIds,getall=True):
	...
	Log(r.url)
	Log(type(r))
	data = json.loads(re.search(rb'ReactDOM\.hydrate\(React\.createElement\(Components\.TrendingGridModule,(.+?)\), document.getElementById',r.content,re.DOTALL).group(1)).get('data')
...
def Getplayers(team=first_teamurl):
	...
	for player in players:
		...
		data = json.loads(re.search(rb'ReactDOM\.hydrate\(React\.createElement\(Components\.PlayerHeroModule,(.+?)\), document.getElementById',r.content,re.DOTALL).group(1)).get('data').get('player')
...
def GetMatchReportImages(url):
	...
	data = json.loads(re.search(rb'ReactDOM\.hydrate\(React\.createElement\(Components\.GalleryModule,(.+?)\), document.getElementById',r.content,re.DOTALL).group(1))
