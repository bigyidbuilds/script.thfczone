...
class WindowMatchCenterLineup(xbmcgui.WindowXML):
	...
	def LineUpView(self,controlId,lineupKey,isSpurs):
		...
		for index,row in enumerate(layout):
			y_pos = int((img_h*index)+(space_h*index))
			space_w = (pitch_w - (len(row)*img_w))/len(row)
			start_pos_x = space_w/2
			for i,item in enumerate(row):
				x_pos = int(start_pos_x+(img_w*i)+(space_w*i)+pitch_x)
		...
		for s,sub in enumerate(subs):
			if isSpurs:
				...
				if self.isHome:
					...
					if img:
						self.addControl(xbmcgui.ControlImage(int(startsubpos_x), int(y_pos),sub_img_w,sub_img_h,img,2))
					self.addControl(xbmcgui.ControlImage(int(startsubpos_x+sub_img_w),int(y_pos+sub_img_h/2),100,20,player_label_bg))
					self.addControl(xbmcgui.ControlLabel(int(startsubpos_x+sub_img_w),int( y_pos+sub_img_h/2),100,20,sub.get('Name').split(' ',1)[-1], font='font10', textColor='0xFF000033', alignment=0x00000002|0x00000004))
				elif self.isAway:
				...
				if not self.isAway:
					...
					self.addControl(xbmcgui.ControlImage(int(startsubpos_x),int(y_pos),100,20,player_label_bg))
					self.addControl(xbmcgui.ControlLabel(int(startsubpos_x), int(y_pos),100,20,sub.get('Name').split(' ',1)[-1], font='font10', textColor='0xFF000033', alignment=0x00000002|0x00000004))
