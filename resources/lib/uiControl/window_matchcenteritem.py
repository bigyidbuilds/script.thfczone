...
from resources.lib.modules._common import Log,DownloadFile,DelAllContents,UrlFileName
...
class WindowMatchCenterItems(xbmcgui.WindowXML):
	...
	def MatchSlides(self,url):
		...
		for slide in slides:
			slide = slide
			imgurl = slide.get('image') .get('url')
			path = os.path.join(self.slide_temp_path,UrlFileName(imgurl))
			threads.append(threading.Thread(target=DownloadFile, args=(imgurl,path)))
