		...
		with self.lsc.lock:
			with self.lsc.conn:
				...
				for data in self.utdata:
					...
					try:
						banner = data.get('items')[0].get('brandingSettings').get('image').get('bannerExternalUrl')
					except:
						banner = None
		...
		with self.lsc.lock:
			with self.lsc.conn:
				...
				for a in podcastrss.PodcastRss().CHANNEL:
					...
					c.execute("INSERT INTO podcast_channels VALUES(?,?,?,?,?,?)",(podcast_id,a.get('title'),a.get('description'),a.get('image'),a.get('rss_url'),None))
